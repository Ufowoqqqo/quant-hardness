...
int main(){try{
 ...
 for(int m:{768,384}){
 faiss::IndexPQ pq(d,m,8);for(float&v:pq.pq.centroids)v=normal(rng);pq.is_trained=true;pq.add(n,x.data());
 ...
 req(graph_fingerprint(graph.hnsw)==fp,"graph changed");std::cout<<"PASS d1536/PQ"<<m<<"; ef32/64/128; full-sort, refinement, native identity and graph invariance\n";
 }
